        ...
        public IEnumerable<Post> GetSuggestions(string id)
        {
            ...
            if (userPosts.Count(up => !up.PostDeleted).Equals(0))
            {
                var hashtags = _appDbContext.Hashtags.GroupBy(h => h.Content)
                    .Select(group => new { Content = group.Key, Count = group.Count() }).OrderByDescending(h => h.Count).Take(3);
                ...
                foreach (var HashTag in hashtags)
                {
                    ...
                    var PostWithHashTag = _appDbContext.Posts.Include("HashTags").Where(p => p.HashTags.Contains(HashTagActual))
            ...
            List<Post> userLikesPosts = new List<Post>();
            ...
            try
            {
                suggestedPosts = (from item in items
                    ...
                    into foundPosts
                                  select foundPosts.OrderBy(x => x.Likes).First()).ToList(); //gets posts that have the same hashtag, but do NOT have the same author 
            }
            catch (Exception e)
            {
